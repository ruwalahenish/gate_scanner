...
Equity chart : equity curve + drawdown % (HTML div for embedding in backtest report)
...
# ---------------------------------------------------------------------------
# Trade chart  (backtest)
# ---------------------------------------------------------------------------

def build_trade_chart(
    symbol: str,
    ohlcv_df: pd.DataFrame,
    trade,         # gate_scanner.backtester.trade.Trade
    out_path: str,
) -> Optional[str]:
    """
    Generate an interactive candlestick chart for a closed backtest trade.

    Shows: trade window (30 bars before entry → 15 bars after exit),
    EMA lines, initial SL / T1 / T2 / T3 levels, trailing-SL step-line,
    ...
    # Slice a window around the trade
    idx       = ohlcv_df.index
    entry_pos = int(idx.searchsorted(trade.entry_date))
    exit_pos  = int(idx.searchsorted(trade.exit_date)) if trade.exit_date else len(idx) - 1
    start     = max(0, entry_pos - 30)
    end       = min(len(idx), exit_pos + 16)
    df        = ohlcv_df.iloc[start:end].copy()
    if len(df) < 2:
        return None
    ...
    # Static price levels
    sl_dist_pct = abs(trade.entry_price - trade.sl_price) / trade.entry_price * 100
    levels: List[Tuple[Optional[float], str, str, str]] = [
        (trade.entry_price, _CLR_ENTRY, f"Entry  ₹{trade.entry_price:.2f}",                "solid"),
        (trade.sl_price,    _CLR_SL,    f"Init SL  ₹{trade.sl_price:.2f}  (−{sl_dist_pct:.1f}%)", "dash"),
        (trade.t1,          _CLR_T1,    f"T1  ₹{trade.t1:.2f}",                            "dot"),
        (trade.t2,          _CLR_T2,    f"T2  ₹{trade.t2:.2f}",                            "dot"),
        (trade.t3,          _CLR_T3,    f"T3  ₹{trade.t3:.2f}",                            "dot"),
    ]
    _add_level_lines(fig, levels, df.index)

    # Trailing SL step-line
    sl_history = getattr(trade, "trailing_sl_history", [])
    sl_dates = [trade.entry_date] + [d for d, _ in sl_history]
    sl_vals  = [trade.sl_price]   + [v for _, v in sl_history]
    if trade.exit_date:
        sl_dates.append(trade.exit_date)
        sl_vals.append(getattr(trade, "trailing_sl", trade.sl_price))
    if len(sl_dates) > 1:
        fig.add_trace(
            go.Scatter(
                x=sl_dates, y=sl_vals,
                mode="lines",
                name="Trailing SL",
                line=dict(shape="hv", color=_CLR_TRAIL, width=1.8, dash="dot"),
    ...
    # Entry marker
    entry_color  = _UP_CLR if pnl_pct >= 0 else _DN_CLR
    entry_symbol = "triangle-up" if pnl_pct >= 0 else "triangle-down"
    fig.add_trace(
        go.Scatter(
            x=[trade.entry_date],
            y=[trade.entry_price],
    ...
    if trade.exit_date and trade.exit_price is not None:
        exit_color = _DN_CLR if reason == "SL" else ("#9E9E9E" if reason == "EOD" else "#FFD700")
        fig.add_trace(
            go.Scatter(
                x=[trade.exit_date],
                y=[trade.exit_price],
                ...
                hovertemplate=f"Exit ₹{trade.exit_price:.2f} [{reason}]<extra></extra>",
            ),
            row=1, col=1,
        )
...
# ---------------------------------------------------------------------------
# Equity / drawdown chart  (backtest summary)
# ---------------------------------------------------------------------------
...
def build_equity_chart(
    ...
    """
    Return an HTML <div> snippet (no <html>/<body> wrapper) suitable for
    embedding directly in the backtest report HTML page.

    Primary y-axis : equity (₹).
    Secondary y-axis : drawdown (%).
    Loads Plotly from CDN (requires internet; cached by browser).
    """
    if not _check_plotly():
        return "<p>Plotly not installed — equity chart unavailable. Run: pip install plotly</p>"
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Equity (₹)",
            mode="lines",
            line=dict(color=_CLR_ENTRY, width=2),
            fill="tozeroy",
    ...
    if not drawdown_series.empty:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="Drawdown (%)",
                mode="lines",
                line=dict(color=_CLR_SL, width=1.5),
                fill="tozeroy",
    ...
    fig.update_layout(
        ...
        plot_bgcolor=_BG_CLR,
        height=320,
        margin=dict(l=65, r=80, t=15, b=40),
        legend=dict(
            orientation="h",
            yanchor="bottom", y=1.01,
            xanchor="left",   x=0,
    ...
    fig.update_yaxes(
        title_text="Equity (₹)", secondary_y=False,
        showgrid=True, gridcolor="#2d2d3d",
    )
    fig.update_yaxes(
        title_text="Drawdown %", secondary_y=True,
        showgrid=False,
    )
    fig.update_xaxes(showgrid=True, gridcolor="#2d2d3d")
