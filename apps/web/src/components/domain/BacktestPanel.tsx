...
type Trade = {
  ...
  pnl_abs: number | null; pnl_pct: number | null;
  holding_days: number;
};
...
export function BacktestPanel() {
  ...
  return (
    <Box>
          ...
          <Grid container spacing={2} alignItems="flex-start">
            ...
            <Grid item xs={12} sm={2}>
              <TextField
                ...
                inputProps={{ min: 10000, step: 50000, inputMode: "numeric" }}
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                ...
                placeholder="RELIANCE,TCS (blank = default)"
                helperText={
                  universeCount
      ...
      {showLiveTable && (
        <Card sx={{ mb: 2 }}>
          <CardContent sx={{ pb: "12px !important" }}>
            ...
            <TableContainer sx={{ maxHeight: 340, overflowX: "auto", WebkitOverflowScrolling: "touch" }} aria-label="Per-stock backtest results">
                    ...
                    return (
                      <TableRow key={`scanning-${sym}`}>
                        ...
                        {Array.from({ length: 9 }).map((_, i) => (
                          <TableCell key={i}><Skeleton variant="text" width={40} /></TableCell>
                        ))}
                      </TableRow>
                    );
                  })}
      ...
      {/* ── Inline per-symbol trades ──────────────────────────────────────── */}
      {selectedSymbol && (
        <Card sx={{ mb: 2 }} ref={inlineTradesRef}>
          <CardContent>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1, flexWrap: "wrap" }}>
              <Typography variant="subtitle2">Trades — {selectedSymbol}</Typography>
              {symbolTradesLoading
                ? <CircularProgress size={14} />
                : <Chip label={symbolTrades.length} size="small" />}
              <Chip
                label="Clear" size="small" variant="outlined"
                onDelete={() => setSelectedSymbol(null)}
            ...
            {symbolTradesLoading ? (
              ...
              <TableContainer sx={{ maxHeight: 360, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow>
      ...
      {/* ── History alongside live results ────────────────────────────────── */}
      {(result || showLiveTable) && history.length > 0 && (
        <Card sx={{ mb: 2 }}>
          <CardContent sx={{ pb: "12px !important" }}>
            <Typography variant="subtitle2" mb={0.5}>Recent Backtests</Typography>
      ...
      {result && (
        <>
          <Grid container spacing={2} mb={2}>
            ...
            <Grid item xs={6} sm={4} md={2}>
              <MetricCard label="Total Trades" value={result.total_trades?.toString() ?? "—"} sentiment="neutral" />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <MetricCard
            ...
            <Card>
              <CardContent>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1, flexWrap: "wrap" }}>
                  ...
                  {selectedSymbol && (
                    <Chip
                      label="Show all" size="small" variant="outlined"
                      onDelete={() => setSelectedSymbol(null)}
                      onClick={() => setSelectedSymbol(null)}
                      sx={{ fontSize: "0.68rem" }}
                    />
                ...
                <TableContainer sx={{ maxHeight: 420, overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                  <Table size="small" stickyHeader>
                    <TableHead>
                      <TableRow>
